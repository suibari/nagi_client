<script lang="ts">
	...
	let botPost = $derived<PostView>({
		uri: `${news.uri}#bot-comment`, cid: news.cid,
		author: botActor ?? { did: 'did:unknown:bot-tan', handle: 'bot-tan', displayName: 'Botたん', isBot: true },
		text: news.botComment, langs: [news.lang], createdAt: news.createdAt, indexedAt: news.indexedAt,
		reactions: [], isBot: true, isAffirmation: false,
	});
...
<article class="news-card">
	...
	<div class="news-actions">
		{#if safeUrl}<a class="primary news-read" href={safeUrl} target="_blank" rel="noopener noreferrer">{m.newsReadArticle()}</a>{/if}
		<button class="ghost icon-action" type="button" onclick={openQuote} aria-label={m.newsQuote()} title={m.newsQuote()}><Icon name="quote" size={18}/></button>
		<button class="ghost icon-action" type="button" onclick={share} aria-label={shared ? m.newsCopied() : m.newsShare()} title={shared ? m.newsCopied() : m.newsShare()}><Icon name="share" size={18}/></button>
	</div>
...
<style>
	.news-card{padding:1rem 1.1rem;border:1px solid var(--line);border-radius:var(--radius-l);background:var(--bg-raised);box-shadow:var(--shadow-card)}
	...
	.news-actions{display:flex;align-items:center;gap:.4rem;flex-wrap:wrap;margin-top:.85rem}.news-actions .news-read{display:inline-flex;align-items:center;min-height:36px;padding:.45rem .9rem;text-decoration:none}.news-actions .icon-action{width:36px;height:36px}

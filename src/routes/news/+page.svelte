<script lang="ts">
	...
	let items = $state<NewsView[]>([]), cursor = $state<string>(), hasMore = $state(false), loading = $state(false), error = $state(''), loadedLang = $state<string>();
	async function load(reset = false) {
		if (loading) return; loading = true; error = '';
		try { const page = await getPositiveNews(i18n.locale, reset ? undefined : cursor); items = reset ? page.items : [...items, ...page.items]; cursor = page.cursor; hasMore = page.hasMore; loadedLang = i18n.locale; }
...
<section class="news-feed" aria-busy={loading}>
	...
	{:else}{#each items as news (news.uri)}<NewsCard {news}/>{/each}{#if hasMore}<button class="more icon-action" disabled={loading} onclick={() => load()} aria-label={m.loadMore()}><Icon name="more" size={20}/></button>{/if}{/if}
</section>

...
<ul>
	...
	<li>
		<strong>The text of "Kossori" posts, and of the diaries for days that contain one.</strong> These
		are not copies but the <strong>only</strong> copy. They are visible to no one but you and Bot-tan.
	...
	<li>
		...
		When the original is a Kossori post, that URI is an identifier issued by the Service that contains no
		author DID, so neither the summary nor a reaction to it can be traced back to the author.
	</li>
...
<ul>
	...
	<li>
		For an "Affirm together" summary: the selected post text and images, quoted post text and
		images, and link-card title and description. <strong
			>The linked page itself is not fetched for this summary.</strong
		>
	</li>
	<li>
...
<ul>
	<li>
		...
		and the post URIs shown in "Affirm together" for seven days
	</li>
...
<div class="callout warn">
	<p>
		<strong>"Kossori" (quiet) posts are readable only by you and Bot-tan.</strong> Their text is stored
		on our servers rather than in your PDS, and never appears in shared timelines, search, threads, or
		anyone else's profile. Only you and Bot-tan can reply.
		<strong>A diary for a day that contains a Kossori post is treated the same way</strong>: other users
		see only the date and the post count (the cell in the annual graph on your profile).
	</p>
	<p>
		However, <strong
			>an anonymous Bot-tan summary — with neither your name nor the original wording — may appear in
			"Affirm together."</strong
		> The original-post URI stored alongside that summary is an identifier issued by the Service that contains
		no author DID. Readers can react to the summary, but cannot trace it back to you or to the original
		text. Channel-only posts may be summarized in the same way, but they are stored in your PDS as ordinary
		public records.
	</p>
	<p>
...
<ul>
	...
	<li>
		...
		registered before Nagi introduced creation markers, and Kossori posts made before 18 August 2026, which
		are public records in your PDS (Kossori posts made since then are not in any PDS, so they do not apply).
		To remove them completely, you must delete them from your PDS yourself.
	</li>
